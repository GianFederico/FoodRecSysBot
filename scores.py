...
def FSAscore(url):
    ...
    recipe_values = filtered_recipe_df.iloc[0][
        ["fat", "saturatedFat", "sugars", "sodium"]
    ].to_dict()
...
def find_scores(url):
    ...
    if not filtered_recipe_df.empty:
        ...
        ingredients_list = [
            ...
            for ingredient in filtered_recipe_df["ingredients"].values[0].split(",")
            ...
            for item in food_commodities:
                if fuzz.partial_ratio(ingredient, item.lower()) >= 90:  # threshold
                    matched_ingredients[item] = {
                        "final_co2": commodity_df.loc[
                            commodity_df["Food commodity ITEM"] == item, "final_co2"
                        ].values[0],
                        "final_wfp": commodity_df.loc[
                            commodity_df["Food commodity ITEM"] == item, "final_wfp"
                        ].values[0],
                    }
                    break
        ...
        if matched_ingredients:
            ...
            for ingredient, values in matched_ingredients.items():
                ...
                iss = calculate_iss(
                    ingredient,
                    float(values["final_co2"].replace(",", ".")),
                    float(values["final_wfp"].replace(",", ".")),
                    min_co2,
                    max_co2,
                    min_wfp,
                    max_wfp,
...
# new_columns = ['ID'] + [f'RICETTA{i}_URL' for i in range(1, 7)] + ['GT_Healthiness', 'GT_Sustainability'] * 6
...
# for index, row in replies_df.iterrows():
#     # Extract ID and URLs from the original row
#     id_ = row['ID']
#     urls = [row[f'RICETTA{i}_URL'] for i in range(1, 7)]

#     # Calculate scores for each URL and flatten the result
#     scores = [score for url in urls for score in find_scores(url)]

#     # Create a new row with ID, URLs, and scores
#     new_row = [id_] + urls + scores

#     # Append the new row to the new DataFrame
#     new_df.loc[len(new_df)] = new_row

# # Save the new DataFrame to a CSV file
# new_df.to_csv('GT_Replies.csv', index=False)
