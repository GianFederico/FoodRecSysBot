...
def get_mood():
    ...
    richVitaminCList_en = [
        "guavas," "kiwis," "red peppers," "strawberries," "oranges." "papaya",
        "broccoli",
        "tomato",
        "peas",
        "kale",
    ...
    def rescore_parameter(
        param, score_update, low, high, score_low, score_mid, score_high
    ):
    ...
    def dfFromIngredient(df, searchIngrendient):
        ...
        for index, row in df.iterrows():
            ...
            if any(
                not (ingredient.lower().find(searchIngrendient))
                for ingredient in listIngredients
            ):
                new_rows.append(row)

        return pd.concat([returnDf, pd.DataFrame(new_rows, columns=df.columns)])

    def rescoreOverweight(row):
        new_score = row.score

        # calories
        new_score = rescore_parameter(
            row.calories,
            new_score,
            low=133.5,
            high=400.5,
    ...
    def rescoreDifficulty(row, difficulty):
        ...
        if difficulty == 3:
            if (
                row.difficulty == "Media"
                or row.difficulty == "Facile"
                or row.difficulty == "Molto facile"
            ):
                new_score = new_score * 2
            else:
                new_score = new_score * 1
        ...
        if difficulty == 4:
            if (
                row.difficulty == "Difficile"
                or row.difficulty == "Media"
                or row.difficulty == "Facile"
                or row.difficulty == "Molto facile"
            ):
                new_score = new_score * 2
        ...
        if difficulty == 5:
            if (
                row.difficulty == "Molto difficile"
                or row.difficulty == "Difficile"
                or row.difficulty == "Media"
                or row.difficulty == "Facile"
                or row.difficulty == "Molto facile"
            ):
    ...
    def rescoreCost(row, cost):
        ...
        if cost == 2:
            if row.cost == "Basso" or row.cost == "Molto basso":
                ...
                new_score = new_score * 1
        if cost == 3:
            if row.cost == "Medio" or row.cost == "Basso" or row.cost == "Molto basso":
                new_score = new_score * 2
            else:
                new_score = new_score * 1
        if cost == 4:
    ...
    isLowNickel = (
        int(request.args.get("isLowNickel"))
        if (request.args.get("isLowNickel") is not None)
        else ""
    )
    isVegetarian = (
        int(request.args.get("isVegetarian"))
        if (request.args.get("isVegetarian") is not None)
        else ""
    )
    isLactoseFree = (
        int(request.args.get("isLactoseFree"))
        if (request.args.get("isLactoseFree") is not None)
        else ""
    )
    isGlutenFree = (
        int(request.args.get("isGlutenFree"))
        if (request.args.get("isGlutenFree") is not None)
        else ""
    )
    isLight = (
        int(request.args.get("isLight"))
        if (request.args.get("isLight") is not None)
        else ""
    )
    isDiabetes = (
        int(request.args.get("isDiabetes"))
        if (request.args.get("isDiabetes") is not None)
        else ""
    )
    isPregnant = (
        int(request.args.get("isPregnant"))
        if (request.args.get("isPregnant") is not None)
        else ""
    )

    user_difficulty = (
        int(request.args.get("difficulty"))
        if (request.args.get("difficulty") is not None)
        else ""
    )

    goal = (
        int(request.args.get("goal")) if (request.args.get("goal") is not None) else ""
    )
    user_cost = (
        int(request.args.get("user_cost"))
        if (request.args.get("user_cost") is not None)
        else ""
    )
    user_time = (
        int(request.args.get("user_time"))
        if (request.args.get("user_time") is not None)
        else ""
    )
    ...
    bmi = (
        request.args.get("fatclass")
        if (request.args.get("fatclass") is not None)
        else ""
    )
    ...
    # # MARKER change score if hour == 'evening' @@@@@@@@@@@@@@@@@@@@????? hour is not present in the request
    # if hour == 'evening':
    #     if lang == 'it':
    #         df.score = df.apply(rescore_bad_food, axis=1, food='caffè')
